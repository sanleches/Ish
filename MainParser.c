...
/* External objects */
extern ish_intg syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern ish_intg line; /* source code line number - defined in scanner.c */
...
extern ish_void startParser(ish_void);
extern ish_intg startScanner(BufferPointer sc_buf);

static ish_void printParserError(ish_thread fmt, ...);
static ish_void displayParser(BufferPointer ptrBuffer);
static ish_long getParserFileSize(ish_thread fname);
static ish_void callGarbageCollector(ish_void);
...
ish_intg mainParser(ish_intg argc, ish_thread* argv) {
...
ish_void printParserError(ish_thread fmt, ...) {
...
ish_long getParserFileSize(ish_thread fname) {
	FILE* input;
	ish_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
ish_void displayParser(BufferPointer ptrBuffer) {
...
ish_void callGarbageCollector(ish_void) {
